...
public class AvroSchemaBuilder implements IAvroSchemaBuilder {
    ...
    public Schema getCacheConfigurationsAvroSchema() {
        ...
        SchemaBuilder.FieldAssembler<Schema> cacheDataTypesFieldAssembler = SchemaBuilder.record(FieldNames.CACHE_DATA_TYPES_FIELD_NAME)
                .namespace(CACHE_DATA_TYPES_NAMESPACE).fields();
        cacheDataTypesFieldAssembler.name(FieldNames.CACHE_KEY_TYPE_FIELD_NAME).type().stringType().noDefault();
        cacheDataTypesFieldAssembler.name(FieldNames.CACHE_VAL_TYPE_FIELD_NAME).type().stringType().noDefault();

        fieldAssembler.name(FieldNames.CACHE_DATA_TYPES_FIELD_NAME).type(cacheDataTypesFieldAssembler.endRecord()).noDefault();
